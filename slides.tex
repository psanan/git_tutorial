...
\item \emph{check out} a branch with \lstinline{git checkout}. This updates \texttt{HEAD} to point to the branch, and updates the working directory to the snapshot indicated by the branch.
\item 
\begin{lstlisting}[language=C++]
...
\item Bitbucket
\begin{itemize}
\item is less focused on ``social'' features.
